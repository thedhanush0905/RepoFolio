          ...
          {portfolios.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {portfolios.map((port) => (
                <div 
                  key={port._id}
                  className="bg-[#0B1117] border border-[#17212B] p-6 rounded-sm flex flex-col justify-between space-y-6 hover:border-[#2b3b4d]/60 transition-colors"
                >
                  <div className="space-y-2">
                    <div className="flex justify-between items-start">
                      <h3 className="text-[#F3F0E8] font-bold text-base tracking-tight font-mono">
                        {port.name}
                      </h3>
                    ...
                    <div className="text-[10px] text-gray-500 font-mono space-y-1">
                      <div>TEMPLATE: <span className="text-[#A8AAA4] uppercase">{port.template}</span></div>
                      <div>LAST UPDATED: <span className="text-[#A8AAA4]">{new Date(port.updatedAt).toLocaleString()}</span></div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-[#17212B] pt-4">
                    ...
                    <button
                      disabled={actionLoading === port._id}
